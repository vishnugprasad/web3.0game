...
import { updateObstacle, setObstacle, getObstacleRect } from "./obstacle.js";
import { updateNft, setupNft, getNftRect } from "./nft.js";
...
function checkLose() {
  ...
  return getObstacleRect().some(rect => isCollision(rect,playerRect))
}
...
function checkIfWeGotNft(){
  const playerRect = getPlayerRect()
  if(getNftRect().some(rect => isCollision(rect,playerRect))){
    const nftToRemove = document.querySelectorAll("[data-nft]")[0]
    nftToRemove.remove()
    nftScore +=1
    nftScoreElem.textContext = `nft score: ${nftScore}`
  }
}
